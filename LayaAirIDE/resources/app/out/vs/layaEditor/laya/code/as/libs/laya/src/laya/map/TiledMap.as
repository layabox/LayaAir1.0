package laya.map {
	...
	public class TiledMap {
		...
		private function onJsonComplete(e:*):void {
			...
			_mapRect.top = Math.floor(_rect.y / _mapTileH);
			_mapRect.bottom = Math.floor((_rect.y + _rect.height) / _mapTileH);
			_mapRect.left = Math.floor(_rect.x / _mapTileW);
			_mapRect.right = Math.floor((_rect.x + _rect.width) / _mapTileW);
		 ...
		 * @return
		 */
		public function getSprite(index:int):GridSprite {
		...
		private function updateViewPort():void {
			...
			var tX:Number = _rect.x - tPaddingRect.x;
			var tY:Number = _rect.y - tPaddingRect.y;
			var tWidth:int = _rect.width + tPaddingRect.width + tPaddingRect.x;
			var tHeight:int = _rect.height + tPaddingRect.height + tPaddingRect.y;
			_mapRect.top = Math.floor(tY / _gridHeight);
			_mapRect.bottom = Math.floor((tY + tHeight) / _gridHeight);
			_mapRect.left = Math.floor(tX / _gridWidth);
			_mapRect.right = Math.floor((tX + tWidth) / _gridWidth);
			clipViewPort();
