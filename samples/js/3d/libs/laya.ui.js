...
(function(window,document,Laya){
	...
	var ColorFilter=laya.filters.ColorFilter,Config=Laya.Config,Ease=laya.utils.Ease,Event=laya.events.Event;
	var EventDispatcher=laya.events.EventDispatcher,Font=laya.display.css.Font,Graphics=laya.display.Graphics;
	var Handler=laya.utils.Handler,Input=laya.display.Input,Loader=laya.net.Loader,MathUtil=laya.maths.MathUtil;
	...
	*
	*/
	//class laya.ui.AnimationPlayerBase extends laya.events.EventDispatcher
	var AnimationPlayerBase=(function(_super){
		...
		__class(AnimationPlayerBase,'laya.ui.AnimationPlayerBase',_super);
		...
		*
		*/
		__proto.displayToIndex=function(value){}
		/**清理。方便对象复用。*/
	...
	*
	*/
	//class laya.ui.KeyFrameAnimation extends laya.ui.AnimationPlayerBase
	var KeyFrameAnimation=(function(_super){
		function KeyFrameAnimation(){
			this.targetDic=null;
			this.animationData=null;
			KeyFrameAnimation.__super.call(this);
			if(KeyFrameAnimation.sortIndexFun==null){
				KeyFrameAnimation.sortIndexFun=MathUtil.SortByKey("index",false,true);
			}
		}

		__class(KeyFrameAnimation,'laya.ui.KeyFrameAnimation',_super);
		var __proto=KeyFrameAnimation.prototype;
		/**
		*初始化动画数据
		*@param targetDic 对象表
		...
		*
		*/
		__proto.setUp=function(targetDic,animationData){
			this._labels=null;
		...
		*/
		__proto.displayToFrame=function(frame){
			...
			if(frame<0)frame=0;
			if(frame>this._count)frame=this._count;
			var nodes=this.animationData.nodes,i=0,len=nodes.length;
			for(i=0;i<len;i++){
				this.displayNodeToFrame(nodes[i],frame);
			}
		}

		/**@inheritDoc */
		__proto.displayToIndex=function(value){
			this.displayToFrame(value);
		}
		...
		*/
		__proto.calculateDatas=function(){
			if(!this.animationData)return;
			var nodes=this.animationData.nodes,i=0,len=nodes.length,tNode;
			this._count=0;
			for(i=0;i<len;i++){
				tNode=nodes[i];
		...
		*/
		__proto.calculateNodeKeyFrames=function(node){
			...
			for(key in keyFrames){
				...
				tKeyFrames.sort(KeyFrameAnimation.sortIndexFun);
		...
		*
		*/
		__proto.dealKeyFrame=function(keyFrame){
		...
		*/
		...
		KeyFrameAnimation.sortIndexFun=null
	...
	var View=(function(_super){
		...
		*/
		__proto.createView=function(uiView){
			if (uiView.animations && !this.idMap)this.idMap={};
			...
			if (uiView.animations){
				...
				var tAniO;
				for (i=0;i < len;i++){
					tAni=new KeyFrameAnimation();
					tAniO=animations[i];
					tAni.setUp(this.idMap,tAniO);
		...
		*/
		...
		View.createComp=function(uiView,comp,view){
			...
			if (uiView.compId&&view&&view.idMap){
				view.idMap[uiView.compId]=comp;
			}
			return comp;
		}
