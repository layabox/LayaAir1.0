...
package
{
	...
	public dynamic class XmlDom
	{
		...
		private function createChild(children:XMLList):void 
		{
			childNodes = [];
			if (children.length() > 0)
			{
				...
				for (var i:int = 0; i < len; i++)
				{
					...
					{
					case "element": 
						var childXml:XmlDom = new XmlDom();
						childXml.nodeType = 1;
						
						childXml.localName = child.localName();
						childXml.nodeName = childXml.localName;
						childXml._ns = child.namespace();
						
						// Create all element node.
						childXml.createNode(child, this);
						
						childElement = childXml;
						break;
					case "text":
						this.parentNode.nodeType = 3;
						this.parentNode.textContent = child;
					default: 
						childElement = child;
					}
		...
		private function buildAttributes(attributeList:XMLList):void 
		{
