...
优化graphics性能
修复cache文字模糊的问题 【感谢：风中雨 】
修复ComboBox自适应弹出方向的bug 【感谢：叮当】
List增加setItem接口，方便列表修改数据
增加更多示例demo
修复WebGL下文字偏移的bug
修复播放Animation不能立即获取宽高的问题
修复Socket类，sendString改为send,新增connectByUrl接口
修复进入输入状态的input元素坐标闪动
修复设置裁剪时获取getBounds不对bug
修复Socket发送二进制数据长度可能不对的问题
修复发布资源裁透明区域裁剪后，按钮和clip显示不正常的bug
修复cacheAs在高清屏下模糊的bug
...
修改默认打开页面布局
当没有页面时不再导出ts和js代码
修复在切换到空项目之后资源加载不对的问题
修复组件缩放后对齐功能不对的问题
修复打开系统窗口后ctrl键状态不对的问题
修复属性面板文字超过区域的问题
修复第一次标尺显示不全的问题
修复快捷工具栏创建目录没反应的问题
修复修改特定属性后点击空白无法取消选中
...
修复提示信息有时移除不了的问题
修复对象设置scale之后编辑区中点击拾取区域没有变的问题
修复关闭页面之后没有清空面板的问题
修复Clip interval特别小时报错的问题
修复在默认属性中设置runtime不起作用的问题
修复右键拖动表现不合理的问题
修复不能在文件夹中创建文件的问题，修复第二次创建页面时上次信息残留的问题
修复配置文件中有特殊字符导致配置文件保存有问题的问题
...
==============编辑器0.9.7==============
更新日志：
重命名页面时提示先关闭打开的页面，以免在重命名页面时可能产生的错误
修复刷新资源无效的问题
转换mornUI项目时scale转scaleX scaleY
修复文本框默认text不起作用的问题


===============类库0.9.6===============
更新日志：
新增TimeLine实现，可以作更为丰富的时间轴动画
调整callLater和Timer的顺序，先执行timer，再执行callLater 【感谢：虎皮猫大人】
优化Timer性能，提高并发Timer的性能
分离事件和渲染逻辑，防止意外的bug
Tween默认加入对象池管理，最大程度利用对象
Input添加最大输入字符限制接口
优化引擎，修改WebGl报错，性能得到提升 【感谢：奇奇】
统计信息增加更多信息的统计，方便排查性能问题
View去掉默认的mouseenable=false
增加cacheAs属性，除了包含cacheAsBitmap功能外，增加了renderTarget类型
增加位图字体根据设置字体大小进行缩放的功能
修复拖动stop时会拖出限制区域区域的bug 【感谢：Mr right】
修复输入框不能手动选中的bug
修复重设窗口大小时，不能repaint的问题
修复added事件和display事件顺序不对的bug 
修复display事件在重复添加到显示列表中时会重复派发的bug 【感谢：JAyDeN】
修复on监听在某些时候未能正确打开mouseenable的bug
修复HTML文本对齐显示问题
修复webGL模式下设置Text的alpha无效 【感谢：木乃伊】
...
==============编辑器0.9.6==============
更新日志：
优化编辑器性能，效率提升50% 【感谢：迷失的贼 】
增加编辑器的Mac适配
增加查找资源引用功能
改进颜色选择框实现方式，比之前更精细 【感谢:小文】
修改编辑器文件夹无法重命名的bug 【感谢：ziv】
修复新建页面时页面选项没有和项目设置同步的问题
修复输入框Tab切换相关bug 【感谢：小破】
修复IDE下将粒子效果拖拽到页面IDE卡死的bug 【感谢：小破】
修复导出默认值不起作用的问题
修复Label默认属性导出不生效的问题 【感谢：JAyDeN】
修复重命名文件夹不对的问题，对color为数字类型的情况作异常判断
修复tab设置LabelColors在layaAir IDE舞台上为全黑显示 【感谢：JAyDeN】
...
优化滤镜及缓存处理逻辑
优化引擎，优化webgl
Loader增加根据图集删除资源的处理 【感谢：蝈蝈】
Sprite的loadImage函数增加加载完成回调
ClassUtils增加类反射方法支持
HttpRequest增加默认头信息
去掉滚动条禁用判断 增加tab的Labels变化后触发resize事件
修复大图合集闪烁问题
修改graphics重设为空IMG标记没有清除的bug
修复HTML排版已知显示错误
修改查询支持函数导致WEBGL无法运行的BUG
修改滚动在屏幕旋转后不对的bug
...
增加图集打包是否是2的幂次方裁剪的选项
修改颜色值因精度问题不对的问题
修复修改资源文件名没有修改页面属性的问题
修复ts头文件没有保存的问题
修复有默认值设置的组件，默认值不起作用的问题
修复js文件发布不对的问题
...
更新日志：
新增对TileMap的支持
优化鼠标事件，处理多点touch对拖拽的影响
改进移动端输入框显示方式
缓动实例增加to和from接口，方便对象复用
List增加只有滚动条才做cache优化的判断
优化引擎代码
修改骨骼动画算法
修复屏幕适配为showall时画布缩放错误
修复清理graphic可能导致渲染出错的问题
增加Hbox，Vbox映射
更改缩放模式常量为小写（参照js标准） 修复某些手机点击和移动容易误操作的bug
修复颜色拾取有时不起作用的问题


已知问题：
Graphics的drawPath无法在WebGL模式下正常工作

==============编辑器0.9.4==============
更新日志：
属性为皮肤的输入框，增加标记并且可以拖拽资源进去，点击此标记也能快速定位资源
增加模板面板，增加部分默认模板
调整编辑器皮肤及布局
修复删除未使用资源不对的问题
修复模板转换不对的问题
修复转换原版Mornui项目转换的问题，适配已经不存在的控件和改名的控件
修复导出代码时默认属性不生效的问题
修复创建页面时设置参考宽高结果不正确的问题
修复修改组件类型后，资源面板图标类型不对的问题
修复新版骨骼动画播放不了的问题
修复了自定义Tab，RadioGroup后，在编辑器内设置selectedIndex无效的bug
修复编辑器内输入框默认padding没生效的问题
修复龙骨动画转换失败的问题（4.1.1版本）


===============类库0.9.3===============
更新日志：
Button和Tab，RadioGroup增加strokeColors属性，可以设置不同状态下的描边颜色（酷鲨 - 方块 - 美术）
playerglobal.swc增加Date类（蝈蝈）
TextInput的Label增加默认padding（1,1,1,3）（酷鲨-方块-美术）
Animation增加fromUrl静态方法，方便创建动画序列帧（Mr right）
HttpRequest类增加头文件设置参数
增加List, Panel在cacheAsBitmap下的优化
增加多图图集的支持
Timer增加清理某个对象身上的所有时间监听方法clearAll
编辑器颜色选取可以滑动选取了（JAyDeN）
对UI组件进行部分优化
List增加cacheContent属性，表示是否缓存内容，如果数据源较少，并且list内无动画，设置此属性为true能大大提高性能
增加失去焦点时自动停止播放背景音乐
增加2D统计模式下增加drawcall
优化Sprite性能
修复on打开鼠标状态失败错误
修复单独设置borderColor无效的bug
修复火狐浏览器宽高显示不正确
修复停止拖动导致对象身上其他缓动被停掉的bug
修复html换行不对的bug
修改Sprite的loadImage方法，增加宽高的参数
修改Loader自动判断资源后缀在有版本参数时识别错误
修改适配方案兼容更多平台
修改graphic缓存bounds可能不对的bug
...
更新日志：
更换编辑器新皮肤，更加漂亮了
增加拖动项目文件到IDE中打开项目的功能
属性面板增加tab快捷键快速切换
编辑器图集打包程序可设置是否为2的幂次方
优化调整坐标时若设置left top时同时修改left top
修改拖动调整大小的实现方式，修复按比例缩放时当图片大小特别小时会导致比例失真的问题
修复标签页切换不对的问题
修复设置页面属性时参考图默认值不对的问题
修复有时点击页面打不开的问题
修改属性面板修改值刷新的方式，提高性能
修改编辑器标签页显示逻辑，修复当前页面标签不可见的问题
修复发布的js ts后缀名不对的问题
修复页面默认属性不起作用的问题
修复转换为容器时顺序被反序的问题
修复显示列表小图标操作不方便的问题
...
加载图集会返回图集json信息
Tab增加stateNum属性
增加批次合并和加亮等绘图模式，提高引擎性能(enableRenderMerge)
增加loader超时，防止大量加载造成引擎卡顿
startDrag增加disableMouseEvents属性，设置为true能提高拖动性能
渲染默认清晰模式，可以设置Laya.stage.useHDRendering更改
修复Input在showAll下坐标偏差
更改下拉框默认字体大小为12号字
修复文本padding属性显示错误
修复tab显示bug
修复加载图集文件不走URL的bug
...
更新日志：
发布时会把未打包的资源拷贝到输出目录
新增层级面板按delete键会删除当前选中节点
修复强制发布失效的bug 
修复按Shift键等比缩放不对的问题
增加图集打包尺寸设置（F9）
Tab和RadioGroup增加stateNum属性
属性配置增加accept属性，可接受资源面板拖入皮肤
选中编辑器组件，增加快速定位到皮肤功能（Ctrl+K）
修复拖动tab报错 增加button在编辑器的预览
修复Shift+拖动缩放比例不对的问题
修改组件配置信息
修复组件默认值不对的问题
集成swf转换，骨骼动画转换工具到编辑器
修复拖动图片到编辑器导致编辑器被覆盖的问题
...
===============类库0.9.1===============
更新日志：
增加LayaAirJS和LayaAirTS版本发布
修复Tab设置水平和垂直布局无效的bug
Text字体默认增加两像素偏移
修复webgl版粒子显示问题
修复canvas粒子表现不对的问题
修改alpha设置为0后无法恢复的bug
输入框位置不对 滚动文本不显示最后一行
修复webaudio没有预加载时未加载成功时播放获取到的channel无效的问题
支持自定义精灵缓存
修复空url的异常
增加alpha的判断，修复alpha特别大的时候显示异常
修改button没有描边效果
修改配置信息
Input调整移动端输入框外观
增加场景是否发生旋转属性
修改屏幕适配方案，增加Browser.width 和height属性来获取浏览器物理宽高
...
修复编译错误
更新属性信息
修复新创建的模板在模板面板小图标不正常的问题 
当标签页过多时，添加显示更多的按钮用来显示被截取的标签页
修复导出配置文件图集宽高错误
调整与底层框架冲突的快捷键
修复更换后缀名之后点击UIView无法打开的问题
修复修改后缀名后转换不正确的问题
修复没有用户自定义ui渲染库时的异常
调整标尺坐标
调整模板目录
