package laya.resource {
	...
	public class Resource extends EventDispatcher implements IDispose {
		...
		public function set memorySize(value:int):void {
			...
			_memorySize = value;
			this.event(Event.MEMORY_CHANGED, offsetValue);//data为内存变化值
		}
		 ...
		 */
		public function activeResource(forceCreate:Boolean = false):void {
			_lastUseFrameCount = Stat.loopCount;
			if (_released || forceCreate) {
				recreateResource();
			}
		}
		 ...
		 * @return 是否成功释放
		 */
		public function releaseResource():Boolean {
			if (lock)//资源管理器为空不受资源管理控制
				return false;
			if (!_released) {
				detoryResource();
				_released = true;
		...
		protected function compoleteCreate():void {
			_released = false;
			this.event(Event.RECOVERED, this);//触发恢复事件,忽略异步影响
		}
	}
}