package laya.webgl.canvas {
	...
	public class WebGLContext2D extends Context {
		...
		public function _drawText(tex:Texture, x:Number, y:Number, width:Number, height:Number, m:Matrix, tx:Number, ty:Number, dx:Number, dy:Number):void {
			...
			tex.active();
			if (GlUtils.fillRectImgVb(_curSubmit._vb || _vb, _clipRect, x + tx, y + ty, width || tex.width, height || tex.height, tex.uv, m || _curMat, _x, _y, dx, dy)) {
				_curSubmit._numEle += 6;
				_maxNumEle = Math.max(_maxNumEle, _curSubmit._numEle);
			}
		}
		...
		override public function drawCanvas(canvas:HTMLCanvas, x:Number, y:Number, width:Number, height:Number):void {
			...
			if (Config.showCanvasMark) {
				...
				strokeRect(x - 1, y - 1, width + 2, height + 2);
		...
		public function strokeRect(x:Number, y:Number, width:Number, height:Number, lineWidth:Number = 1):void {
			line(x, y, x + width, y, lineWidth / 2, _curMat);
			line(x + width, y, x + width, y + height, lineWidth / 2, _curMat);
			line(x, y, x, y + height, lineWidth / 2, _curMat);
			line(x, y + height, x + width, y + height, lineWidth / 2, _curMat);
		}
		...
		public function lineTo(x:Number, y:Number):void {
			line(_other.path._x, _other.path._y, x, y, _other.lineWidth / 2, _curMat);
		}
		...
		public function drawPoly(x:Number, y:Number, r:Number, edges:int, boderColor:uint, lineWidth:Number, color:uint):void {
			_path.polygon(x, y, r, edges, color, lineWidth ? lineWidth : 1, boderColor);
