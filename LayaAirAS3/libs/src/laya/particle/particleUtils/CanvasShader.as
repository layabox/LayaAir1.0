package laya.particle.particleUtils
{
	...
	public class CanvasShader
	{
		...
		public function ComputeParticlePosition(position:Float32Array, velocity:Float32Array, age:Number, normalizedAge:Number):Float32Array
		{
			...
			var radius:Number = MathUtil.lerp(a_RadiusRadian[0], a_RadiusRadian[1], normalizedAge); //计算粒子受半径和角度影响（无需计算角度和半径时，可用宏定义优化屏蔽此计算）
			var radianHorizontal:Number = a_RadiusRadian[2] * normalizedAge;
			var radianVertical:Number = a_RadiusRadian[3] * normalizedAge;
		...
		public function ComputeParticleColor(projectedPosition:Float32Array, color:Float32Array, normalizedAge:Number):Float32Array
		{
		...
		public function getData(age:Number):Array
		{
			...
			v_Color = ComputeParticleColor(gl_Position, a_Color, normalizedAge);
