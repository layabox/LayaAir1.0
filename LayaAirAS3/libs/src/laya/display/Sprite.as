package laya.display {
	...
	public class Sprite extends Node implements ILayout {
		 ...
		 *
		 */
		private function transPointList(pList:Array, x:Number, y:Number):void {
		...
		public function boundPointsToParent(ifRotate:Boolean = false):Array {
			...
			if (!transform) {
				transPointList(pList, this.x - pX, this.y - pY);
				return pList;
			}
		...
		/** @private */
		public function _removeRenderType(type:int):void {
			((_renderType & type) == type) && (_renderType &= ~type);
		}
		
		/** @private */
		public function _addRenderType(type:int):void {
			_renderType |= type;
		}
		...
		override public function childChanged(child:Node = null):void {
			...
			//TODO:LINSHI
			//if (numChildren) _renderType |= RenderSprite.CHILDS;
