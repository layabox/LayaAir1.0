package laya.display {
	...
	public class Graphics {
		...
		public function clear():void {
			...
			_sp._renderType &= ~RenderSprite.IMAGE;
			_repaint();
		}
		...
		private function getCmdPoints():Array {
			...
			for (var i:int = 0, n:int = cmds.length; i < n; i++) {
				...
				case context._scale: 
					...
					switchMatrix(tMatrix, tempMatrix);
					break;
				...
				case context._drawTexture: 
					if (cmd[3] && cmd[4]) {
						addPointArrToRst(rst, Rectangle._getBoundPointS(cmd[1], cmd[2], cmd[3], cmd[4]), tMatrix);
					} else {
						var tex:Texture = cmd[0];
						addPointArrToRst(rst, Rectangle._getBoundPointS(cmd[1], cmd[2], tex.width, tex.height), tMatrix);
					}
					break;
				case context._drawRect: 
				case context._fillRect: 
					addPointArrToRst(rst, Rectangle._getBoundPointS(cmd[0], cmd[1], cmd[2], cmd[3]), tMatrix);
					break;
				case context._drawCircle: 
				case context._fillCircle: 
					addPointArrToRst(rst, Rectangle._getBoundPointS(cmd[0] - cmd[2], cmd[1] - cmd[2], cmd[2] + cmd[2], cmd[2] + cmd[2]), tMatrix);
					break;
				case context._drawLine: 
					addPointArrToRst(rst, [cmd[0], cmd[1], cmd[2], cmd[3]], tMatrix);
					break;
				case context.drawCurves: 
					//addPointArrToRst(rst, [cmd[0], cmd[1]], tMatrix);
					//addPointArrToRst(rst, cmd[2], tMatrix, cmd[0], cmd[1]);
					addPointArrToRst(rst, Bezier.I.getBezierPoints(cmd[2]), tMatrix, cmd[0], cmd[1]);
					break;
				...
				case context._drawPath: 
					//addPointArrToRst(rst, [cmd[0], cmd[1]], tMatrix);
					addPointArrToRst(rst, getPathPoints(cmd[2]), tMatrix, cmd[0], cmd[1]);
					break;
				case context._drawPie: 
				case context._drawPieWebGL: 
					addPointArrToRst(rst, getPiePoints(cmd[0], cmd[1], cmd[2], cmd[3], cmd[4]), tMatrix);
					break;
		...
		private function switchMatrix(tMatix:Matrix, tempMatrix:Matrix):void {
			tempMatrix.concat(tMatix);
			tempMatrix.copy(tMatix);
		}
		
		private static function addPointArrToRst(rst:Array, points:Array, matrix:Matrix, dx:Number = 0, dy:Number = 0):void {
			var i:int, len:int;
			len = points.length;
			for (i = 0; i < len; i += 2) {
		...
		private static function addPointToRst(rst:Array, x:Number, y:Number, matrix:Matrix):void {
		...
		private function textureLoaded(tex:Texture, param:Array):void {
			tex.off(Event.LOADED, this, textureLoaded);
			param[3] = tex.width;
			param[4] = tex.height;
			_repaint();
		}
		...
		public function drawCurves(x:Number, y:Number, points:Array, lineColor:*, lineWidth:Number = 1):void {
			var arr:Array = [x + 0.5, y + 0.5, points, lineColor, lineWidth];
			_saveToCmd(Render.context.drawCurves, arr);
		}
		...
		private function getPiePoints(x:Number, y:Number, radius:Number, startAngle:Number, endAngle:Number):Array {
			var rst:Array;
			rst = [x, y];
		...
		public function drawPoly(x:Number, y:Number, points:Array, fillColor:*, lineColor:* = null, lineWidth:Number = 1):void {
			...
			_saveToCmd(Render.context._drawPoly, arr);
		}
		
		private function getPathPoints(paths:Array):Array {
			var i:int, len:int;
			var rst:Array = [];
			len = paths.length;
			var tCMD:Array;
