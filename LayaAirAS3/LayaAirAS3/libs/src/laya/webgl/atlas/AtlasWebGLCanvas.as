	...
	public class AtlasWebGLCanvas extends Bitmap {
		...
		public function texSubImage2D(source:Bitmap, xoffset:Number, yoffset:Number, bitmap:*):void {
			if (!Render.isFlash) {
				...
				(xoffset + 1 <= source.width) && (_flashCacheImage.image.copyPixels(bmData, 0, 0, bmData.width + 1, bmData.height, xoffset, yoffset));
				(yoffset - 1 >= 0) && (_flashCacheImage.image.copyPixels(bmData, 0, 0, bmData.width, bmData.height - 1, xoffset, yoffset));
				(yoffset + 1 <= source.height) && (_flashCacheImage.image.copyPixels(bmData, 0, 0, bmData.width + 1, bmData.height, xoffset, yoffset));
				_flashCacheImage.image.copyPixels(bmData, 0, 0, bmData.width, bmData.height, xoffset, yoffset);
