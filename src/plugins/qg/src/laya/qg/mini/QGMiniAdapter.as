package laya.qg.mini {
	...
	public class QGMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false, isSon:Boolean = false):void {
			...
			if (!window.hasOwnProperty('qg') &&window.navigator.userAgent.indexOf('OPPO') < 0) return;
			isZiYu = isSon;
			isPosMsgYu = isPosMsg;
			EnvConfig = {};
			...
			if (!isZiYu) {
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(QGMiniAdapter, onMkdirCallBack));
			}
			//系统信息			
...
//			window.CanvasRenderingContext2D.prototype = window.qg.createCanvas().getContext('2d').__proto__;
			...
			window.qg.onMessage && window.qg.onMessage(_onMessage);
		}
		...
		public static function pixelRatio():Number {
			if (!EnvConfig.pixelRatioInt) {
				try {
					systemInfo.pixelRatio = window.devicePixelRatio;//xiaosong add
		...
		private static function postInfoToContext(url:String, atlaspngUrl:String, atlasJson:Object):void {
			...
			if (fileNativeUrl) {
				window.qg.postMessage && window.qg.postMessage({url: url, atlasdata: postData, imgNativeUrl: fileNativeUrl, imgReadyUrl: textureUrl, isLoad: "opendatacontext"});
			} else {
		...
		public static function sendSinglePicToOpenDataContext(url:String):void {
			...
			if (fileNativeUrl) {
				window.qg.postMessage && window.qg.postMessage({url: url, imgNativeUrl: fileNativeUrl, imgReadyUrl: url, isLoad: "openJsondatacontextPic"});
			} else {
				throw "获取图集的磁盘url路径不存在！";
			}
		}
		...
		public static function sendJsonDataToDataContext(url:String):void {
			if (!QGMiniAdapter.isZiYu) {
				...
				if (atlasJson) {
					window.qg.postMessage && window.qg.postMessage({url: url, atlasdata: atlasJson, isLoad: "openJsondatacontext"});
				} else {
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
			}
