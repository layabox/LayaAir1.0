package laya.qg.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = QGMiniAdapter.window.qg.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = QGMiniAdapter.window.qg.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = QGMiniAdapter.nativefiles.length;i<sz;i++)
			{
		...
		public static function downOtherFiles(fileUrl:String, callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,isAutoClear:Boolean = true):void {
			wxdown({url: fileUrl, success: function(data:Object):void {
				...
				trace("====downloadfile  fail：" + JSON.stringify(data));
				callBack != null && callBack.runWith([1, data]);
			}});
		}
		...
		{
			if(QGMiniAdapter.window.navigator.userAgent.indexOf('QGGame') <0)
			{
		...
		{
			var memSize:int = QGMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				QGMiniAdapter.window.qg.postMessage && QGMiniAdapter.window.qg.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = QGMiniAdapter.window.qg.env.USER_DATA_PATH + value;
		}
	}
}
