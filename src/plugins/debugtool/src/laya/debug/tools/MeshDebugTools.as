package laya.debug.tools 
{
	 ...
	 * @author ww
	 */
	public class MeshDebugTools 
	{
		
		public static var meshList:Array = [];
		
		public static function workMesh(mesh:MeshData):void
		{
			if (mesh["inList"]) return;
			mesh["inList"] = true;
			meshList.push(mesh);
			trace("ver:");
			trace(mesh.vertices.join(","));
			trace("uvs:");
			trace(mesh.uvs.join(","));
		}
		public static function showToStage():void
		{
			Laya.stage.on(Event.MOUSE_DOWN, null, onMouseDown);
		}
		private static var tI:int = 0;
		private static function onMouseDown():void
		{
			if (meshList.length < 0) return;
			tI = tI % meshList.length;
			showMeshToStage(meshList[tI]);
			tI++;
		}
		private static var _meshView:MeshDebugView;
		public static var pos:Point = new Point();
		private static function showMeshToStage(mesh:MeshData):void
		{
		  if (!_meshView) _meshView = new MeshDebugView();
		  _meshView.showMesh(mesh);
		  _meshView.pos(pos.x, pos.y);
		  Laya.stage.addChild(_meshView);
		  
		}
		public static function drawVerticles(verticles:Array,scaleX:Number=1,scaleY:Number=1,sp:Sprite,offX:Number=0,offY:Number=0,r:Number=1,color:String="#ff0000",dT:Number=0):Sprite
		{
			sp = sp||new Sprite();
			var g:Graphics;
			g = sp.graphics;
