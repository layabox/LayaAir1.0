package laya.bd.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = BMiniAdapter.window.swan.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = BMiniAdapter.window.swan.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = BMiniAdapter.nativefiles.length;i<sz;i++)
			{
		...
		{
			var memSize:int = BMiniAdapter.minClearSize;
...
//			}});
			...
			{
				BMiniAdapter.window.swan.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
...
//				trace("--------tempFilesListStr:" + tempFilesListStr);
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = BMiniAdapter.window.swan.env.USER_DATA_PATH + value;
		}
	}
}
