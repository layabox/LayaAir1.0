package laya.mi.mini{
	...
	public class KGMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(window.navigator.userAgent.indexOf('QuickGame') <0) return;
			isZiYu = isSon;
			isPosMsgYu = isPosMsg;
			EnvConfig = { };
			//设置资源存储目录
			if(!isZiYu)
			{
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(KGMiniAdapter, onMkdirCallBack));
			}
			if(!isZiYu)
			{
				systemInfo = KGMiniAdapter.window.qg.getSystemInfoSync();
...
//			MiniLocalStorage.__init__();
		...
		private static function onReciveData():void
		{
			...
			{
				KGMiniAdapter.window.qg.onMessage(function(message:Object):void{
...
//			node.type = {};//xiaosong
		...
		private static function postInfoToContext(url:String,atlaspngUrl:String,atlasJson:Object):void
		{
			...
			if(fileNativeUrl)
			{
				var openDataContext:* =KGMiniAdapter.window.qg.getOpenDataContext();
