package laya.mi.mini{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* =  KGMiniAdapter.window.qg.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = KGMiniAdapter.window.qg.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = KGMiniAdapter.nativefiles.length;i<sz;i++)
			{
				//这里不嫩改成 ==0 因为是模糊匹配
		 ...
		 * @param fileType 文件类型
		 */
		public static function downFiles(fileUrl:String, encoding:String = "utf8", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = "",isAutoClear:Boolean =true):void {
			var downloadTask:* = __JS__('qg').downloadFile({url: fileUrl, success: function(data:Object):void {
		...
		public static function downOtherFiles(fileUrl:String, callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,isAutoClear:Boolean = true):void {
			__JS__('qg').downloadFile({url: fileUrl, success: function(data:Object):void {
				if (data.statusCode === 200) {
		...
		{
			var memSize:int = KGMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				KGMiniAdapter.window.kg && KGMiniAdapter.window.kg.postMessage && KGMiniAdapter.window.kg.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = KGMiniAdapter.window.qg.env.USER_DATA_PATH + value;
		}
	}
}
