package laya.d3.core {
	...
	public class HeightMap {
		 ...
		 * @param height 高度图高度。
		 */
		public static function creatFromMesh(meshSprite:MeshSprite3D, width:int, height:int, outCellSize:Vector2):HeightMap {
			var vertices:Vector.<Vector.<Vector3>> = new Vector.<Vector.<Vector3>>();
			var indexs:Vector.<Uint16Array> = new Vector.<Uint16Array>();
			
			var mesh:Mesh = meshSprite.mesh as Mesh;
			...
			var cellWidth:Number = outCellSize.elements[0] = widthSize / width;
			var cellHeight:Number = outCellSize.elements[1] = heightSize / height;
			var halfCellWidth:Number = cellWidth / 2;
			var halfCellHeight:Number = cellHeight / 2;
			...
			for (var w:int = 0; w < width; w++) {
				var posZ:Number = minZ + w * cellHeight + halfCellHeight;
				heightMap._datas[w] = [];
				for (var h:int = 0; h < height; h++) {
					var posX:Number = minX + h * cellWidth + halfCellWidth;
