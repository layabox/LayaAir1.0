package physics {
	...
	public class CubeEditCubeCollider extends CubePhysicsCompnent{
		...
		public var collisionCube:Vector3 = new Vector3();
		public var cubeProperty:int = 0;
		...
		public function dataAdd(x:int, y:int, z:int,color:int):void
		{
			var key:int = (x >> 5) + ((y >> 5) << 8) + ((z >> 5) << 16);
			var o:* = data[key] || (data[key] = {});
			o[x % 32 + ((y % 32) << 8) + ((z % 32) << 16)] = (colorIndex & 0xff000000) >> 24;
		}
		...
		public function InitCubemap(cubemap:CubeMap,cubeMeshManager:CubeMeshManager):void
		{
			...
			for (var i:int = 0, n:int = cubeinfos.length; i <n ; i++) {
				dataAdd(cubeinfos[i].x+vector.x, cubeinfos[i].y+vector.y, cubeinfos[i].z+vector.z,cubeinfos[i].color);
			}
