package laya.layagl.cmdNative {
	 ...
	 * 填充文字命令
	 */
	public class FillWordsCmdNative {
		...
		public static function create(words:Array, x:Number, y:Number, font:String, color:String):FillWordsCmdNative {
			if (!cbook) new Error('Error:charbook not create!');
			var cmd:FillWordsCmdNative = Pool.getItemByClass("FillWordsCmd", FillWordsCmdNative);
			...
			cmd.createFillText(cmd._draw_texture_cmd_encoder_, words as Vector.<HTMLChar>, x, y, font, color);
		...
		private function createFillText(cbuf:*, data:Vector.<HTMLChar>, x:Number, y:Number, font:String, color:String):void {
			...
			cbook.filltext_native(ctx, null, data, x, y, font, color, null, 0, null);
		}
		/**
		 * 回收到对象池
		 */
