...
(function(window,document,Laya){
	...
	Laya.interface('laya.resource.ITextureProxy');
	...
	*/
	...
	var Graphics=(function(){
		...
		*/
		__proto.drawTriangles=function(texture,x,y,points){
			if (!texture || !points || points.length % 4 !=0 || points.length < 4){
				return;
			}
			this._saveToCmd(Render._context._fillTrangles,[texture,0,0,points,null]);
		}
		...
		__getset(1,Browser,'container',function(){
			...
			if (!Browser._container){
				if(Laya.EnvConfig.container){
					Browser._container=Laya.EnvConfig.container;
				}
				else{
		...
		__proto.load=function(url,type,cache,group,ignoreCache){
			...
			if (!this._http){
				if (Laya.EnvConfig.HttpRequest){
					this._http=new Laya.EnvConfig.HttpRequest();
					}else {
	...
	*/
	...
	var Sprite=(function(_super){
		...
		__proto.destroy=function(destroyChild){
			...
			this._transform=null;
			if (this._texture){
				this.offTextureEvent(this._texture);
		...
		*texture proxy 下载完毕
		*/
		__proto.onTextureProxyLoaded=function(){
			if (this._texture){
				this.graphics.cleanByTexture(this._texture.texture,0,0);
			}
		}
		...
		*texture proxy被销毁
		*/
		__proto.onTextureProxyDisposed=function(){
			this.graphics.cleanByTexture(null,0,0);
		}

		/**
		*texture proxy更新内容
		*/
		__proto.onTextureProxyUpdate=function(){
			if (this._texture){
				this.graphics.cleanByTexture(this._texture.texture,0,0);
			}
		}
		...
		*/
		...
		__getset(0,__proto,'texture',function(){
			...
			if (value){
				this.addTextureEvent(value);
				value.addRef();
			}
			this._texture=value;
			if (this._texture){
				this.graphics.cleanByTexture(value.texture,0,0);
			}
		});
