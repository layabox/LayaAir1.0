package laya.d3.core.material {
	...
	public class EffectMaterial extends BaseMaterial {
		/**渲染状态_加色法混合。*/
		public static const RENDERMODE_ADDTIVE:int = 0;
		/**渲染状态_透明混合。*/
		public static const RENDERMODE_ALPHABLENDED:int = 1;
		
		/** 默认材质，禁止修改*/
		public static const defaultMaterial:EffectMaterial = new EffectMaterial();
		...
		public static const MAINTEXTURE:int = Shader3D.propertyNameToID("u_AlbedoTexture");
		public static const TINTCOLOR:int = Shader3D.propertyNameToID("u_AlbedoColor");
		...
		public function set texture(value:BaseTexture):void {
			if (value)
				_defineDatas.add(EffectMaterial.SHADERDEFINE_MAINTEXTURE);
			else
				_defineDatas.remove(EffectMaterial.SHADERDEFINE_MAINTEXTURE);
		...
		public function set tilingOffset(value:Vector4):void {
			if (value) {
				if (value.x != 1 || value.y != 1 || value.z != 0 || value.w != 0)
					_defineDatas.add(EffectMaterial.SHADERDEFINE_TILINGOFFSET);
				else
					_defineDatas.remove(EffectMaterial.SHADERDEFINE_TILINGOFFSET);
			} else {
				_defineDatas.remove(EffectMaterial.SHADERDEFINE_TILINGOFFSET);
			}
