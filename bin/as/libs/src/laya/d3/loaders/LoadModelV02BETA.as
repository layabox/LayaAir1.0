package laya.d3.loaders {
	...
	public class LoadModelV02BETA {
		...
		/**@private */
		private var _decIDToVertexDeclaration:Object;
		/**@private */
		private var _decIDToVertexbufferData:Object;
		/**@private */
		private var _decIDToSubMesh:Object;
		/**@private */
		 ...
		 */
		public function LoadModelV02BETA(readData:Byte, version:String, mesh:Mesh, materials:Vector.<BaseMaterial>, materialMap:Object) {
			_decIDToVertexDeclaration = {};
			_decIDToVertexbufferData = {};
			_decIDToSubMesh = {};
			_mesh = mesh;
		...
		private function _onLoaded(readData:Byte):Mesh {
			...
			for (var key:String in _decIDToVertexbufferData) {
				var VerbufferData:Float32Array = _decIDToVertexbufferData[key];
				var vertexDeclaration:VertexDeclaration = _decIDToVertexDeclaration[key];
				var VertexBuffer:VertexBuffer3D = VertexBuffer3D.create(vertexDeclaration, (VerbufferData.length * 4) / vertexDeclaration.vertexStride, WebGLContext.STATIC_DRAW, true);
				VertexBuffer.setData(VerbufferData);
				
				var subMeshs:Array = _decIDToSubMesh[key];
				for (i = 0, n = subMeshs.length; i < n; i++)
					(subMeshs[i] as SubMesh)._vertexBuffer = VertexBuffer;
			}
			return _mesh;
		}
		...
		public function READ_SUBMESH():Boolean {
			...
			var indexBufferOffset:int, i:int, n:int;
			...
			if (vertexBufferData) {//TODO:
				...
				_decIDToVertexbufferData[vertexDecID] = mergeData;
				
				_decIDToSubMesh[vertexDecID].push(submesh);
			} else {
				indexBufferOffset = 0;
				_decIDToVertexbufferData[vertexDecID] = new Float32Array(vbArrayBuffer);
				_decIDToVertexDeclaration[vertexDecID] = vertexDeclaration;
				_decIDToSubMesh[vertexDecID] = [];
				_decIDToSubMesh[vertexDecID].push(submesh);
			}
			
			var vertexElements:Array = vertexDeclaration.getVertexElements();
			...
			var indexBufferData:Uint16Array = new Uint16Array(ibArrayBuffer);
