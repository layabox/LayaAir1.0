	...
	public class Text extends Sprite {
		...
		public function typeset():void {
			...
			if (overflow != VISIBLE) {
				...
				endLine = Math.min(endLine, func((height - padding[0] - padding[2]) / (leading + fontSize)));
			}
		...
		private function evalTextSize():void {
			...
			for (var n:int = 0, len:int = _lines.length; n < len; ++n) {
				...
				_textWidth = Math.max(getTextWidth(word) + padding[3] + padding[1], _textWidth);
			}
			...
			if (_currBitmapFont) {
				...
				_textHeight = _lines.length * (fontSize + leading) + padding[0] + padding[2];
		}
		...
		protected function parseLines(text:String):void {
			...
			for (var i:int = 0, n:int = lines.length; i < n; i++) {
				...
				if (needWordWrapOrTruncate)
					parseLine(lines[i], wordWrapWidth);
				else
					_lines[i] = lines[i];
			}
		}
		...
		protected function parseLine(line:String, wordWrapWidth:Number):void {
			...
			charsWidth = _currBitmapFont ? _currBitmapFont.getMaxWidth() : Browser.context.measureText("阳").width;
