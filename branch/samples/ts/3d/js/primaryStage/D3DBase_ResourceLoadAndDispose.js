var ResourceLoadAndDispose = (function () {
    function ResourceLoadAndDispose() {
        //前言:3D资源为高级类型,需调用Laya.loader.create()接口创建，基本类型如JSON，BUFFER,TEXT请使用Laya.loader.load()加载;
        Laya3D.init(0, 0);
        var processHandler = Laya.Handler.create(this, onProcessChange, null, false); //创建进度处理Handler，注意：后面要对应释放
        //1:批量加载复杂模式。
        //Laya.loader.create([
        ...
        //var mesh:Laya.Sprite3D = Laya.loader.create(meshUrl, null, null, Mesh);//复杂加载。
        ...
        var material = StandardMaterial.load(materialUrl); //简单加载，为复杂加载的封装。
        //加载Texture2D文件。
    ...
    ResourceLoadAndDispose.prototype.onProcessChange = function (process) {
        trace("Process:", process);
    };
    return ResourceLoadAndDispose;
}());
