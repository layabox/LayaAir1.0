	...
	{		// ======================================================================
		...
		public function assemble( mode:String, source:String, version:uint=1, ignorelimits:Boolean=false ):ByteArray
		{
			...
			for ( i = 0; i < lng && _error == ""; i++ )
			{
				...
				var opFound:OpCode = OPMAP[opCode[0]];//LAYA:TODO:修改字典
				...
				for ( var j:int = 0; j < regLength; j++ )
				{
					...
					var regFound:Register = REGMAP[res[ 0 ]];//LAYA:TODO:修改字典
					...
					if ( isRelative )
					{
						...
						var regFoundRel:Register = REGMAP[relname[0]];	//LAYA:TODO:修改字典					
						if ( regFoundRel == null )
						{ 
					...
					} else
					{
						if ( isSampler )
						{
							...
							for ( k = 0; k<optsLength; k++ )
							{
								if ( verbose )
									trace( "    opt: "+opts[k] );
								var optfound:Sampler = SAMPLEMAP[opts[k]];//LAYA:TODO:修改字典
								if ( optfound == null )
								{
		...
		//	Constants
		// ----------------------------------------------------------------------
		private static const OPMAP:Object					= new Object(); 
		private static const REGMAP:Object					= new Object();
